...
export function jsonError(error: unknown) {
  ...
  return { status: 500, body: { ok: false, code: "internal_error", message } };
}

export function requireHeaders(c: Context, requirements: ReadonlyArray<{ header?: string; name?: string }>) {
  for (const requirement of requirements || []) {
    const header = requirement.header || requirement.name;
    if (header && !c.req.header(header)) {
      throw new HttpError(428, "missing_required_header", \`Missing required header \${header}\`);
    }
  }
}
...
export function requireRequestFields(route: any, input: Record<string, unknown>) {
  for (const field of route.requestContract?.fields || []) {
    if (field.required && (input[field.name] === undefined || input[field.name] === null || input[field.name] === "")) {
      throw new HttpError(400, "missing_required_field", \`Missing required field \${field.name}\`);
    }
  }
}
